...
{
	m_Animation[name] = aiImportFile(FileName, aiProcess_ConvertToLeftHanded);	//場合によっては0に
...
void CAnimationModel::Unload()
{
	for (unsigned int m = 0; m < m_Scene->mNumMeshes; m++)
	{
		m_VertexBuffer[m]->Release();	//頂点バッファリリース
		m_IndexBuffer[m]->Release();	//インデックスバッファリリース
	}
...
void CAnimationModel::Update(const char *AnimationName1, const char *AnimationName2, int Frame, float rate)
{
	...
	for (unsigned int c = 0; c < animation1->mNumChannels; c++)
	{
		...
		aiVector3D pos = pos1 * (1.0f - rate) + pos2 * rate;	//線形補間
		aiQuaternion rot;
		aiQuaternion::Interpolate(rot, rot1, rot2, rate);		//球面線形補間
...
void CAnimationModel::UpdateBoneMatrix(aiNode* node, aiMatrix4x4 matrix)
{
	...
	//マトリクスの乗算順番に注意(親のマトリクス->子のマトリクス->オフセットマトリクス の順 (assimpのみ))
