...
public:
	...
	{
		//判定用の座標を更新
		arm.z = PosPlayer.z - cos(RotPlayer.y*D3DX_PI / 180)*500.0f;
		arm.x = PosPlayer.x - sin(-RotPlayer.y*D3DX_PI / 180)*500.0f;
		arm.y = PosPlayer.y + 600.0f;
		armstrong.z = PosPlayer.z - cos(RotPlayer.y*D3DX_PI / 180)*700.0f;
		armstrong.x = PosPlayer.x - sin(RotPlayer.y*D3DX_PI / 180)*700.0f;
		armstrong.y = PosPlayer.y + 600.0f;

		leg.z = PosPlayer.z - cos(RotPlayer.y*D3DX_PI / 180)*500.0f;
		leg.x = PosPlayer.x - sin(RotPlayer.y*D3DX_PI / 180)*500.0f;
		leg.y = PosPlayer.y + 300.0f;
		legstrong.z = PosPlayer.z - cos(RotPlayer.y*D3DX_PI / 180)*700.0f;
		legstrong.x = PosPlayer.x - sin(RotPlayer.y*D3DX_PI / 180)*700.0f;
	...
	void Colision(D3DXVECTOR3 arms, int damage ,bool attacks, bool Down,bool shoot,int number,int eneattack,int id)
	{
		...
		if (PointtoRectangley(arms, PosPlayer, SclPlayer, attacks, 200.0f, 450.0f) == true)
		{
			...
			ChengeAnimation(id, ANI_HIT, 0.6f);
		}
	}
...
void Player_Update(void)
{
	...
	//1Pの処理
	{
		if (g_player[0].GetDamaged() == false && g_player[0].Getattack() == false)
		{
			ChengeAnimation(0, ANI_WAIT, 0.3f);
			//移動処理 (左)
			if (Keyboard_IsPress(DIK_D) || GamePad_IsPress(0, BUTTON_LEFT))
			{
				...
				ChengeAnimation(0, ANI_MOVE, 0.3f);
			}
			...
			if (Keyboard_IsTrigger(DIK_Y))
			{
				...
				{//足攻撃にする
					...
					ChengeAnimation(0, ANI_KICK, 1.0f);
				}
				else
				{
					//前の攻撃がパンチだった時
					...
					ChengeAnimation(0, ANI_PUNCH, 1.5f);
				}
			}
			...
			if (Keyboard_IsTrigger(DIK_H))
			{
				...
				ChengeAnimation(0, ANI_KICK, 1.5f);
			...
			if (Keyboard_IsTrigger(DIK_U))
			{
				...
				g_player[0].Setcombotime(5.0f);
				PlaySound(SOUND_LABEL_SE_PUNCH_MISS);

				ChengeAnimation(0, ANI_STRONGPUNCH, 1.5f);
			}
			...
			if (Keyboard_IsTrigger(DIK_J))
			{
				...
				ChengeAnimation(0, ANI_STORNGKICK, 1.5f);
	...
	{
	case WAITTIME:
		ChengeAnimation(1, ANI_WAIT, 0.3f);
		break;

	case BACKMOVE:
		ChengeAnimation(1, ANI_MOVE, 0.3f);
	...
	case PUNCH:
		...
		if (g_player[1].cooldowncheck() == true)
		{
			if (g_player[1].CheckCombo() > 1)
			{
				...
				ChengeAnimation(1, ANI_KICK, 1.5f);
				PlaySound(SOUND_LABEL_SE_KICK_MISS);
			}
			else
			{
				...
				g_player[1].Setcombotime(2.0f);
				g_player[1].SetBeforAttack(PPUNCH);
				ChengeAnimation(1, ANI_PUNCH, 1.5f);
				PlaySound(SOUND_LABEL_SE_PUNCH_MISS);
			}
		}
		break;
	...
	//当たり判定
	g_player[0].Colision(g_player[1].GetAttackPattan(), g_player[1].GetAttackpower(), g_player[1].Gethit(),g_player[1].GetDownAttack(),false,0,g_player[1].GetBeforAttack(),0);
	g_player[1].Colision(g_player[0].GetAttackPattan(), g_player[0].GetAttackpower(), g_player[0].Gethit(), g_player[0].GetDownAttack(),false,0, g_player[0].GetBeforAttack(),1);
	...
	for (int i = 0; i < MAX_BULLET; i++)
	{
		...
		{
			g_player[0].Colision(GetBulletPos(i), 100, true, false,true,i, g_player[1].GetBeforAttack(),0);
			g_player[1].Colision(GetBulletPos(i), 100, true, false,true,i, g_player[0].GetBeforAttack(),1);
		}
	}
