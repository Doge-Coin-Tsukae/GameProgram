...
// マクロ定義
//*****************************************************************************
#define	TEXTURE_SHADOW		"asset/texture/shadow/shadow000.jpg"	// 読み込むテクスチャファイル名
#define	SHADOW_SIZE_X		(1000.0f)							// 弾の幅
#define	SHADOW_SIZE_Z		(1000.0f)							// 弾の高さ

#define SHADOW_NUM_VERTEX	4
#define SHADOW_NUM_POLYGON	2

#define	MAX_SHADOW			(16)							// 影最大数
...
// プロトタイプ宣言
//*****************************************************************************
HRESULT static MakeVertexShadow(LPDIRECT3DDEVICE9 pDevice);
...
void Shadow_Draw(void)
{
	...
	//減算合成		(A+B)
	pDevice->SetRenderState(D3DRS_BLENDOP, D3DBLENDOP_REVSUBTRACT);	//結果 = 転送先(DEST) - 転送元(SRC)		//REVSUBTRACT = A + BをA - Bにする
	...
	for (int nCntShadow = 0; nCntShadow < MAX_SHADOW; nCntShadow++)
	{
		if (g_aShadow[nCntShadow].bUse)
		{
			...
			//スケールを反映
			D3DXMatrixScaling(&mtxScl, g_aShadow[nCntShadow].scl.x, g_aShadow[nCntShadow].scl.y, g_aShadow[nCntShadow].scl.z);		//ヨー、ピッチ、ロール	ヨーとロールが同じ動きになること=ジンバルロック
			D3DXMatrixMultiply(&g_aShadow[nCntShadow].mtxWorld, &g_aShadow[nCntShadow].mtxWorld, &mtxScl);
			
			//回転を反映
			D3DXMatrixRotationYawPitchRoll(&mtxRot, g_aShadow[nCntShadow].rot.y, g_aShadow[nCntShadow].rot.x, g_aShadow[nCntShadow].rot.z);		//ヨー、ピッチ、ロール	ヨーとロールが同じ動きになること=ジンバルロック
			D3DXMatrixMultiply(&g_aShadow[nCntShadow].mtxWorld, &g_aShadow[nCntShadow].mtxWorld, &mtxRot);				//回転*ワールド
...
void Shadow_SetPosition(int nIdxShadow, D3DXVECTOR3 pos)
{
	...
	g_aShadow[nIdxShadow].pos.y = 1.0f;
}
