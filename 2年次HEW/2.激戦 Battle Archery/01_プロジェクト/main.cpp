...
定数定義
------------------------------------------------------------------------------*/
#define CLASS_NAME     "BattleArchely"       // ウインドウクラスの名前
#define WINDOW_CAPTION "激闘!バトルアーチェリー" // ウィンドウの名前
...
D3DVIEWPORT9 allscreen = { 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, 0.0f, 1.0f };
...
bool Initialize(HINSTANCE hInst)
{
	...
	////PlaySound(SOUND_LABEL_WEATHER_THUNDER_BGM);

	Fade_Initialize();
	Scene_Initialize(SCENE_INDEX_LOGO);
...
void Finalize(void)
{
	//StopSound(SOUND_LABEL_WEATHER_THUNDER_BGM);
...
void Draw(void)
{
	...
	pD3DDevice->SetViewport(&allscreen);

	// 画面のクリア
	pD3DDevice->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_RGBA(200, 255, 255, 255), 1.0f, 0);
