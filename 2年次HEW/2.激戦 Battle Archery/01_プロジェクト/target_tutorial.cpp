...
// マクロ定義
//*****************************************************************************
#define NUM_TARGET_TUTORIAL 20								//表示的の総数
#define MAX_HP	   1
...
public:
	...
	void Update()
	{
		...
		for (int i = 0; i < BULLET_NUM; i++)
		{
			...
			{
				//判定がTrueだったら
				if (CircleAndPoint(PosTgt, GetBulletPos(i) - (-GetOne(i) * j), radius) &&			//Z座標
...
void Target_Tutorial_Update(void)
{
	...
	for (int i = 0; i < NUM_TARGET_TUTORIAL; i++)
	{
		...
		if (g_TargetTutorial[i].Getlive() == true)
		{
			...
			//回転を反映
			/*この関数は原則1本の軸の動かせる*/
			D3DXMatrixRotationYawPitchRoll(&mtxRot, g_TargetTutorial[i].GetRot().y, g_TargetTutorial[i].GetRot().x, g_TargetTutorial[i].GetRot().z);		//ヨー、ピッチ、ロール	ヨーとロールが同じ動きになること=ジンバルロック
			D3DXMatrixMultiply(&g_TargetTutorial[i].g_mtxWorldField, &g_TargetTutorial[i].g_mtxWorldField, &mtxRot);				//回転*ワールド

			//移動を反映
			D3DXMatrixTranslation(&mtxTranslate, g_TargetTutorial[i].GetPos().x, g_TargetTutorial[i].GetPos().y, g_TargetTutorial[i].GetPos().z);
			D3DXMatrixMultiply(&g_TargetTutorial[i].g_mtxWorldField, &g_TargetTutorial[i].g_mtxWorldField, &mtxTranslate);			//移動*ワールド

			//スケールを反映
			D3DXMatrixScaling(&mtxScl, g_TargetTutorial[i].GetScl().x, g_TargetTutorial[i].GetScl().y, g_TargetTutorial[i].GetScl().z);
			D3DXMatrixMultiply(&g_TargetTutorial[i].g_mtxWorldField, &g_TargetTutorial[i].g_mtxWorldField, &mtxScl);				//拡大・縮小*ワールド
